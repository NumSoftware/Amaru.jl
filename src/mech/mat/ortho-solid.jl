...
mutable struct OrthotropicIpState<:IpState
    ...
    nplanes::Int64  # planes of failure
    active_planes::Array{Int64,1} # indices for active failure planes
...
mutable struct Orthotropic<:Material
    ...
    function Orthotropic(;E=NaN, nu=0.0, ft=0.0, fc=0.0, fu=0.0, epsc=0.0, epsu=0.0, ηn=0.001, ηs=0.5)
        @assert E>0.0
        @assert 0.0<=nu<0.5
        @assert fc>0.0
        @assert ft>0.0
        ...
        this = new(E, nu, ft, fc, fu, epsc, epsu, ηn, ηs)
        return this 
    end
end
...
function load_function(σ::Tensor2)
    j2d = J2D(σ)
    j1  = J1(σ)
    α   = -1.0
    return √j2d + α*j1
...
function uniaxial_young_modulus(mat::Orthotropic, εp::Float64, gamma1::Float64=1.0)
    # εp : principal strain
    # σp : principal stress

    gamma = 1.0
    εp = gamma1*gamma*εp
    εu = gamma1*gamma*mat.εu
...
function gamma1(mat::Orthotropic, σp1::Float64, σp2::Float64)
    ...
    gamma1 = p1λ[2] * (σp2-p2λ[1]) * (σp2-p3λ[1]) / ( (p1λ[1]-p2λ[1]) * (p1λ[1]-p3λ[1]) ) +
             p2λ[2] * (σp2-p1λ[1]) * (σp2-p3λ[1]) / ( (p2λ[1]-p1λ[1]) * (p2λ[1]-p3λ[1]) ) +
             p3λ[2] * (σp2-p1λ[1]) * (σp2-p2λ[1]) / ( (p3λ[1]-p1λ[1]) * (p3λ[1]-p2λ[1]) )
...
function fixD(mat, D::Array{Float64,2}, active_planes::Array{Int64,1})
...
#=
function rotate_tensor(T::Tensor2, σp::Float64)
    σ = [ σp[1]         σp[6]/sqrt(2) σp[5]/sqrt(2) ;
          σp[6]/sqrt(2) σp[2]         σp[4]/sqrt(2) ;
          σp[5]/sqrt(2) σp[4]/sqrt(2) σp[3]         ] 

    S = T*σ
    SS = [ S[1,1] S[2,2] S[3,3] S[2,3]*sqrt(2) S[1,3]*sqrt(2) S[2,1]*sqrt(2) ]
    σp = SS    
    return σp
end
=#
...
function calcD(mat::Orthotropic, ipd::OrthotropicIpState)
    ...
    if f<fmax
        ...
        if ipd.nplanes==0
            σp, V = eig(ipd.σ)
            tensor_rot!(V, R)
        elseif ipd.nplanes==1
            Z = ipd.V1
            σp, V = eigen_with_fixed_dir(ipd.σ, Z)
            tensor_rot!(V, R)
        elseif ipd.nplanes>=2
        ...
        Ep1 = uniaxial_young_modulus(mat, εp[1], gamma1)
        Ep2 = uniaxial_young_modulus(mat, εp[2], gamma1)
        Ep3 = uniaxial_young_modulus(mat, εp[3], gamma1)

        nactive = length(ipd.active_planes)
