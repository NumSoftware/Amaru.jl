...
"""
    save(mesh, filename, verbosity=1)

Saves a mesh object into a file. Available formats are vtu and vtk.
"""
function save(mesh::AbstractMesh, filename::String; verbosity=0)
    ...
    verbosity>0 && printstyled( "  file $filename saved \e[K \n", color=:cyan)
end
...
"""
function Mesh(filename::String; verbosity=0, reorder=false)
    verbosity = clamp(verbosity, 0,2)
