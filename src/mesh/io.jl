...
"""
    save(mesh, filename, printlog=false)

Saves a mesh object into a file. Available formats are vtu and vtk.
"""
function save(mesh::AbstractMesh, filename::String; printlog=true)
    ...
    printlog && printstyled( "  file $filename saved \e[K \n", color=:cyan)
end
...
"""
function Mesh(filename::String; reorder=false, printlog=false)
    
    formats = (".vtk", ".vtu", ".tetgen")

    printlog && printstyled("Mesh loading: filename $filename\n", bold=true, color=:cyan)
    ...
    if format==".vtk"
        printlog && print("  Reading VTK legacy format...\n")
        mesh = read_vtk(filename)
    elseif format==".vtu"
        printlog && print("  Reading VTU format...\n")
        mesh = read_vtu(filename)
    elseif format==".tetgen"
        printlog && print("  Reading tetgen output files...\n")
    ...
    if reorder
        printlog && print("  reordering points...\r")
        reorder!(mesh)
    end
