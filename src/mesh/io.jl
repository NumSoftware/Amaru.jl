...
"""
    save(mesh, filename, report=false)

Saves a mesh object into a file. Available formats are vtu and vtk.
"""
function save(mesh::AbstractDomain, filename::String; report=false)
    formats = (".vtk", ".vtu") 
    ...
    report && printstyled( "  file $filename saved \e[K \n", color=:cyan)
end
...
"""
function Mesh(filename::String; reorder=false, report=false)
    
    formats = (".vtk", ".vtu", ".tetgen")

    report && printstyled("Mesh loading: filename $filename\n", bold=true, color=:cyan)
    ...
    if format==".vtk"
        report && print("  Reading VTK legacy format...\n")
        mesh = read_vtk(filename)
    elseif format==".vtu"
        report && print("  Reading VTU format...\n")
        mesh = read_vtu(filename)
    elseif format==".tetgen"
        report && print("  Reading tetgen output files...\n")
    ...
    if reorder
        report && print("  reordering points...\r")
        reorder!(mesh)
    end
