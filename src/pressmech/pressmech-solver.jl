...
function pm_mount_global_matrices(dom::Domain, ndofs::Int, Δt::Float64)
    ...
    for elem in dom.elems
        ...
        if has_conductivity_matrix
            ...
            Uw = [ dof.vals[:up] for node in elem.nodes for dof in node.dofs if dof.name==:up ]
            RHS[rmap] -= Δt*(H*Uw)
        end

        # Assemble the compressibility matrix
        if has_mass_matrix
            Cpp, rmap, cmap =  elem_mass_matrix(elem)
            nr, nc = size(Cpp)
            for i in 1:nr
                for j in 1:nc
                    push!(R, rmap[i])
...
function pm_solve_step!(G::SparseMatrixCSC{Float64, Int}, DU::Vect, DF::Vect, nu::Int)
    #  [  G11   G12 ]  [ U1? ]    [ F1  ]
    #  |            |  |     | =  |     |
    #  [  G21   G22 ]  [ U2  ]    [ F2? ]
...
"""
    pm_solve!(domain, bcs, options...) -> Bool
...
function pm_solve!(
    ...
    if !isnan(end_time)
        end_time > env.t || error("pm_solve! : end_time ($end_time) is greater that current time ($(env.t))")
        time_span = end_time - env.t
    else
        end_time = env.t + time_span
    end
    isnan(time_span) && error("pm_solve!: neither time_span nor end_time were set.")
    ...
    if save_outs
        ...
        strip(outdir) == "" && (outdir = ".")
        isdir(outdir) || error("pm_solve!: output directory <$outdir> not fount")
    ...
    while T < 1.0 - Ttol
        ...
        for it=1:maxits
            ...
            pm_solve_step!(G, ΔUi, R, nu)   # Changes unknown positions in ΔUi and R
